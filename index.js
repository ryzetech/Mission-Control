...
function timediff(timestamp1ornow, timestamp2, short) {
  ...
  else return `${negative ? "-" : ""}${days} Days, ${hours} Hours, ${minutes} Minutes, ${seconds} Seconds`;
}

// Check if user is still on server, if not remove from db
async function checkUserIsStillHere(usr, usrCheck, timeOffset = 10) {
  if (!usrCheck) {
    if (new Date(usr.updatedAt) < new Date().setDate(new Date() - timeOffset)) return false;
  }
...
function fetchdata() {
  CoinGeckoClient.coins
    ...
    .fetch("ethereum", { localization: "false", tickers: false, community_data: false, developer_data: false, sparkline: false })
    .then((d) => {
...
function startsWithInArray(string, stringArray) {
  for (const element of stringArray) {
    if (string.startsWith(element)) return element;
  }
  return undefined;
}
...
function csprng(minimum, maximum) {
  ...
  // protectng me from my own stupidity
  if (minimum >= maximum) {
    console.error('[CSPRNG] maximum <= minimum');
    return false;
  }
  // protectng me from my own stupidity again
...
client.on("guildMemberAdd", async (member) => {
  ...
  const response = await axios.post("https://dvs.stefftek.de/api/bans", {
  ...
  // if the user is known to Virgin Slayer:
    ...
    const muro = member.guild.roles.cache.get(muterole);
...
client.on("messageCreate", async (message) => {
  ...
  // PING
    ...
    const diff = sent.createdTimestamp - timestamp;
  ...
  // AVATAR
    ...
    // identify user
    const argument = userident(message);
  ...
  // ANIMAL
    ...
    // birb is not a typo, it's the actual name of the endpoint (for real telk???)
    const animals = ["dog", "cat", "panda", "fox", "koala", "birb"];

    const arg = message.content.slice(8).toLocaleLowerCase();

    if (animals.includes(arg)) {
      message.channel.sendTyping();
      const res = await fetch("https://some-random-api.ml/animal/" + arg);
      ...
      let foo = "";
      for (const i in animals) foo += i == 0 ? animals[i] : ", " + animals[i];
      foo += ", red panda";
  ...
  // POKEDEX / POKEMON
    ...
    const arg = message.content.slice(9);

    // do this because this might take a while, people hate it when the bot is sitting around doing seemingly nothing
    message.channel.sendTyping();
  ...
  // MC
    ...
    const arg = message.content.slice(4);
    message.channel.sendTyping();
  ...
  // AVATAR MOD
    ...
    let msg;
    const usr = userident(message);
    let args = message.content.slice(7).toLowerCase();
    const directly_available = [
      "glass",
      "wasted",
      "greyscale",
      "invert",
    ...
    // filter handling
      ...
      args = startsWithInArray(args, directly_available);
      msg = new Discord.MessageEmbed()
        .setColor(embedColorStandard)
        .setAuthor("AvatarMod", embedPB)
  ...
  // HACKER NEWS
    ...
    if (!value) {
      ...
      // update cache                             TTL   h   m    s   = 2h
      const success = botCacheStorage.set("news", json, 2 * 60 * 60);
    ...
    while (i < ycomb_story_amount) {
      ...
      if (!data) {
        ...
        // update the cache
        const success = botCacheStorage.set(
          `news_${i}`,
          data,
          2 * 60 * 60,
        );
      ...
      const url = data.url ? "[**Link**](" + data.url + ")\n" : "";
  ...
  // NASA INFO
    ...
    const args = message.content.slice(6).toLowerCase();
    ...
    if (args.startsWith("apod")) {
      ...
      if (!apod_json) {
        const apod_res = await fetch(`https://api.nasa.gov/planetary/apod?api_key=${nasa_auth}`);
        apod_json = await apod_res.json();
        nasa_rate = parseInt(apod_res.headers.raw()["x-ratelimit-remaining"][0]);

        // set cache                                          TTL   h    s   = 1h
        const success = botCacheStorage.set("nasa-apod", apod_json, 60 * 60);
        if (!success) {
          console.error(
            'ERR [EXEC] "' +
            message.content +
    ...
    // rover info
      ...
      // define cams
      const available_cams = ["mast", "chemcam", "mahli", "mardi", "fhaz", "rhaz", "navcam", "random"];
      ...
      // get rover manifest
      let man_json = botCacheStorage.get("nasa-manifest-curiosity");

      // if the manifest has not been cached yet:
      if (!man_json) {
        const man_res = await fetch(`https://api.nasa.gov/mars-photos/api/v1/manifests/curiosity?api_key=${nasa_auth}`);
        man_json = await man_res.json();

        // set cache                                                     TTL   h    s   = 1h
        const success = botCacheStorage.set("nasa-manifest-curiosity", man_json, 60 * 60);

        if (!success) {
          console.error(
            'ERR [EXEC] "' +
            message.content +
            '" - Error: Cache error! Failed to set cache value for "nasa-manifest-curiosity"',
          );
        }
      }

      // reverse the order to get the latest image
      const cam_data = man_json.photo_manifest.photos.reverse();
      let sol = 0;
      ...
      return message.channel.send({ embeds: [
        new Discord.MessageEmbed()
          .setColor("#0b3d91")
          .setAuthor("NASA Rover", "https://botdata.ryzetech.live/perma/NASA.png")
          .setTitle(`Curiosity ${cam.toUpperCase()} @ SOL ${sol}`)
          .setImage(img)
          .setTimestamp()
          .setFooter(`Requested by ${message.author.tag}`),
      ] });
  ...
  // WOLFRAM
    ...
    const input = message.content.slice(9);

    message.channel.sendTyping();

    const res = await axios.get(`https://api.wolframalpha.com/v2/query?input=${encodeURIComponent(input)}&appid=${wolfram_auth}`);
    ...
    if (data.assumptions !== undefined) {
      ...
      for (let i = 0; i < parseInt(data.assumptions[0].$.count); i++) {
        const ass = data.assumptions[0].assumption[i];
    ...
    const podcount = (parseInt(data.$.numpods) <= wolfram_maxpods) ? parseInt(data.$.numpods) : wolfram_maxpods;

    for (let i = 0; i < podcount; i++) {
      const pod = data.pod[i];
  ...
  // BANANO
    ...
    CoinGeckoClient.coins
      .fetch("banano", {})
      .then((d1) => {
        const stuff = d1.data.market_data;
        const ban_img = d1.data.image.large;

        // yes we need this, no i don't know how to do this better
        CoinGeckoClient.coins
          .fetchMarketChart("banano", { vs_currency: "eur", days: "1" })
          .then((d2) => {
            // get data list
            const prices = d2.data.prices;
            ...
            // expand graph view
            const diff = (highest - lowest) / 10;
            highest += diff;
            lowest -= diff;

            // i know that this is horrible but yolo
            const img = ImageCharts()
              .cht("ls")
              // chart type
              .chf("bg,s,2f313600")
              // background (alpha set to 0, can be changed later)
              .chxt("y")
              // visible axis
              .chxs("0,FFFFFF")
              // axis color
              .chxr("0," + lowest.toFixed(4) + "," + highest.toFixed(4))
              // axis range
              .chg("0,50,1,1,FFFFFF")
              // grid style
              .chtt("Banano 24h €")
  ...
  // note: this is pretty much a "command subcommand" section because i need some subcommands again later for future item buying and selling
      ...
      CoinGeckoClient.coins
        ...
        .then((d2) => {
          ...
          const img = ImageCharts()
            ...
            .chxs("0,FFFFFF")
            // axis color
            .chxr("0," + Math.floor(lowest) + "," + Math.ceil(highest))
            // axis range
            .chg("0,50,1,1,FFFFFF")
            // grid style
            .chtt("Ethereum 24h €")
    ...
    // BUY FOR AMOUNT
      ...
      // parse amount
      const amount = parseFloat(args.slice(7).replace(",", "."));
    ...
    // BUY AMOUNT
      ...
      // parse amount
      const amount = parseFloat(args.slice(4).replace(",", "."));
    ...
    // SELL FOR AMOUNT
      ...
      // parse amount
      const amount = parseFloat(args.slice(8).replace(",", "."));
    ...
    // SELL AMOUNT
      ...
      // parse amount
      const amount = parseFloat(args.slice(5).replace(",", "."));
  ...
  // SEND
    ...
    const pattern = new RegExp(prefix + "send <@!(\\d+)> (\\d*\\.?\\d{1,2})", "s");
    ...
    // Check if any value is not initialized
  ...
  // LEADERBOARD
    ...
    const topEth = await prisma.user.findMany({
      orderBy: { eth: "desc" },
      take: 10,
    });
  ...
  // COINFLIP
    ...
    // parse amount
    const side = args.slice(0, 5).toLowerCase();
    const amount = parseFloat(args.slice(6).replace(",", "."));
  ...
  // TODO comment this, it is highly retarded
    ...
    if (!message.member.roles.cache.some((role) => modroles.includes(role.id))) {return;}
  ...
  else if (Math.round(Math.random() * 4 + 1) === 5 && !message.author.bot) {
    // eslint-disable-next-line no-unused-vars
